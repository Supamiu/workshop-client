...
export class SinglePlayerGameComponent implements OnInit {
    ...
    play_game(numplayer: number, numturn: number, startTime: number): void {
        ...
        if (this.couleurTour === 2) {
            //2 est le joueur, on ne fait rien
        } else {
            setTimeout(() => {
    ...
    placePawn(x: number, y: number): void {
        if (this.couleurTour === 2) {
            ...
            if (checked.result) {
                ...
                this.endGame("Victoire par timeout, " + this.getPlayer(1).name + " a Gagné");
            } else {
                this.play_game(this.couleurTour, this.turn, this.timer.getTime() - 10000);
            }
        }
    }
